...
# ═══════════════════════════════════════════════════════════════════════════════
# ACCESS KEY GATE
# Set ACCESS_KEY in .streamlit/secrets.toml or Streamlit Cloud secret manager.
# Example secrets.toml:   ACCESS_KEY = "my-secret-key-2025"
# ═══════════════════════════════════════════════════════════════════════════════

def check_access_key(entered: str) -> bool:
    try:
        correct = st.secrets["ACCESS_KEY"]
        return entered.strip() == correct.strip()
    except Exception:
        return False
...
if not st.session_state.access_granted:
    ...
    with gate_col:
        ...
        <div style="margin-top:80px;text-align:center;">
            <div style="font-size:2.4rem;color:var(--accent);margin-bottom:6px;">&#x2B21;</div>
            ...
            <div style="font-family:'DM Mono',monospace;font-size:0.72rem;color:var(--muted);
                ...
                Panel Regression Engine &middot; Access Required
            </div>
        </div>
        <div style="margin-top:36px;background:var(--surface2);border:1px solid var(--border);
                    border-radius:10px;padding:28px 28px 24px;">
            <div style="font-size:0.65rem;text-transform:uppercase;letter-spacing:0.12em;
                        color:var(--muted);margin-bottom:10px;">Access Key</div>
        """, unsafe_allow_html=True)
        ...
        if st.session_state.access_error:
            st.markdown(f"""
            <div style="margin-top:10px;padding:8px 12px;background:rgba(240,92,124,0.08);
                        border:1px solid rgba(240,92,124,0.3);border-radius:4px;
        ...
        st.markdown("""
            <div style="margin-top:16px;font-size:0.62rem;color:var(--muted);
                        text-align:center;line-height:1.6;">
                This tool is access-controlled.<br>
                Contact the administrator for your key.
            </div>
        </div>
        """, unsafe_allow_html=True)
    ...
    if unlock_btn:
        if not entered_key:
            ...
            st.rerun()
        elif check_access_key(entered_key):
